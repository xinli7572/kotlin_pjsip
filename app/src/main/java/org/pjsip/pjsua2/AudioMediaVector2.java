/* ----------------------------------------------------------------------------
 * This file was automatically generated by SWIG (http://www.swig.org).
 * Version 3.0.10
 *
 * Do not make changes to this file unless you know what you are doing--modify
 * the SWIG interface file instead.
 * ----------------------------------------------------------------------------- */

package org.pjsip.pjsua2;

public class AudioMediaVector2 {
  private transient long swigCPtr;
  protected transient boolean swigCMemOwn;

  protected AudioMediaVector2(long cPtr, boolean cMemoryOwn) {
    swigCMemOwn = cMemoryOwn;
    swigCPtr = cPtr;
  }

  protected static long getCPtr(AudioMediaVector2 obj) {
    return (obj == null) ? 0 : obj.swigCPtr;
  }

  protected void finalize() {
    delete();
  }

  public synchronized void delete() {
    if (swigCPtr != 0) {
      if (swigCMemOwn) {
        swigCMemOwn = false;
        pjsua2JNI.delete_AudioMediaVector2(swigCPtr);
      }
      swigCPtr = 0;
    }
  }

  public AudioMediaVector2() {
    this(pjsua2JNI.new_AudioMediaVector2__SWIG_0(), true);
  }

  public AudioMediaVector2(long n) {
    this(pjsua2JNI.new_AudioMediaVector2__SWIG_1(n), true);
  }

  public long size() {
    return pjsua2JNI.AudioMediaVector2_size(swigCPtr, this);
  }

  public long capacity() {
    return pjsua2JNI.AudioMediaVector2_capacity(swigCPtr, this);
  }

  public void reserve(long n) {
    pjsua2JNI.AudioMediaVector2_reserve(swigCPtr, this, n);
  }

  public boolean isEmpty() {
    return pjsua2JNI.AudioMediaVector2_isEmpty(swigCPtr, this);
  }

  public void clear() {
    pjsua2JNI.AudioMediaVector2_clear(swigCPtr, this);
  }

  public void add(AudioMedia x) {
    pjsua2JNI.AudioMediaVector2_add(swigCPtr, this, AudioMedia.getCPtr(x), x);
  }

  public AudioMedia get(int i) {
    return new AudioMedia(pjsua2JNI.AudioMediaVector2_get(swigCPtr, this, i), false);
  }

  public void set(int i, AudioMedia val) {
    pjsua2JNI.AudioMediaVector2_set(swigCPtr, this, i, AudioMedia.getCPtr(val), val);
  }

}
